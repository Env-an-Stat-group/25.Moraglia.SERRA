...
#load FANOVA results
load('FANOVA_Results_DallasMarch_IID_HRRR_ReflGamma.RData')
#fanova_results = readRDS('FANOVA_Results_Dallas_IID_2.0_HRRR.rds')
...
for(t in 1:timepoints)
{
  full_ID = inla_output[[t]]$b4.field$ID
  ID = unique(inla_output[[t]]$b4.field$ID)
  betas = inla_output[[t]]$b4.field$mean
  lb = inla_output[[t]]$b4.field$`0.025quant`
  ub = inla_output[[t]]$b4.field$`0.975quant`
  estimate<-c()
  upper = c()
  lower = c()
  #print(paste(t, length(betas)/replicates))
...
saveRDS(fanova_results, file = 'DallasMarch_FANOVA_Results_HRRR_ReflGamma.rds')