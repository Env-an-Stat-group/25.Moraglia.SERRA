...
#load data
fanova_dat = readRDS('FANOVA_DataJulyPRCPTotalNonZero1mm_Dallas_HRRR_D02.rds')
...
#load FANOVA results
load('FANOVA_Results_DallasMarchPRCPTotalNonZero1mm_SPDE_HRRR_D02.RData')
...
#   #print(index)
# }
for(loc in 1:dim(coords)[1])
{
  index = seq(loc, (dim(coords)[1]+16)*replicates, (dim(coords)[1]+16))
  estimate[loc] = mean(betas[index], na.rm = TRUE)
  lower[loc] = mean(lb[index], na.rm = TRUE)
  upper[loc] = mean(ub[index], na.rm = TRUE)
}

all_betas[,t] = estimate
all_lower[,t] = lower
all_upper[,t] = upper
...
saveRDS(fanova_results, file = 'DallasJulyPRCPTotalNonZero1mm_FANOVA_Results_SPDE_D02.rds')